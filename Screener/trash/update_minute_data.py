...
class UpdateMinuteData:
    ...
    def get_last_minute_data(field, tickers_dict, timestamp, previous_data):
        return pd.DataFrame(
            data={k: v[field] for k, v in tickers_dict.items()},
            index=[timestamp],
    ...
    def update_time_frames_data_new(self, previous_data, minute_queue):
        ...
        tickers_queue_data = self.get_tickers_queue_data(minute_queue)
        ...
        for field, time_frame in prev_data_keys:
            for timestamp in tickers_queue_data:
                ...
                if last_label != last_index and last_label not in previous_data[field][time_frame].index:
        ...
        return modified_time_frames, (previous_data['close']['1min'].index[-1] + datetime.timedelta(minutes=1)).minute
