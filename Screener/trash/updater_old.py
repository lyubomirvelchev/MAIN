...
from monitoring.prepare_previous_data import PreparePreviousData
from monitoring.process_manager import AggregateCalculator, SocketMonitor
from monitoring.update_minute_data import UpdateMinuteData
...
PARAMS = {'indicator_params':
    {
        ...
        #     'alerts': {'<': 3000000}},
        # 4: {"name": "MA", "params": {"win_size": 40, "field": 'close', "time_frame": '1min'},
        #     'alerts': {'<': 3000000}},
        # 1: {"name": "EMA", "params": {"win_size": 21, "field": 'open', "time_frame": '1min'},
        #     'alerts': {'<': 3000000}},
        # 2: {"name": "EMA", "params": {"win_size": 9, "field": 'low', "time_frame": '1min'},
        #     'alerts': {'<': 3000000}},
        # 3: {"name": "EMA", "params": {"win_size": 18, "field": 'high', "time_frame": '1min'},
        #     'alerts': {'<': 3000000}},
        # 4: {"name": "EMA", "params": {"win_size": 6, "field": 'close', "time_frame": '1min'},
        #     'alerts': {'<': 3000000}},
        5: {"name": "EMA", "params": {"win_size": 21, "field": 'open', "time_frame": '1min'},
            'alerts': {'<': 3000000}},
        6: {"name": "EMA", "params": {"win_size": 9, "field": 'low', "time_frame": '1min'},
            'alerts': {'<': 3000000}},
        7: {"name": "EMA", "params": {"win_size": 18, "field": 'high', "time_frame": '1min'},
            'alerts': {'<': 3000000}},
        8: {"name": "EMA", "params": {"win_size": 6, "field": 'close', "time_frame": '1min'},
            'alerts': {'<': 3000000}},
        9: {"name": "RSI", "params": {"win_size": 14, "field": 'low', "time_frame": '1min'},
            'alerts': {'<': 3000000}}},
...
class MonitorManger:
    ...
    def __init__(self, tickers, params, api_key=API_KEY):
        ...
        socket = SocketMonitor(self.tickers, api_key)
        socket.run_and_listen()

        agg_calc = AggregateCalculator(socket.raw_queue)
        ...
        self.current_minute = prep_vals.current_time.minute
        ...
        while True:
            if self.aggr_sec_queue.qsize() != 0 or self.minute_queue.qsize() != 0:
                ...
                """HackerMan"""
                socket.socket_monitor.terminate()
                agg_calc.aggregate_process.terminate()
                time.sleep(1)

                socket = SocketMonitor(self.tickers, self.api_key)
                socket.run_and_listen()
                agg_calc = AggregateCalculator(socket.raw_queue)
                self.minute_queue = agg_calc.minute_queue
                self.aggr_sec_queue = agg_calc.aggr_sec_queue
    ...
    def init_all_indicator(self, params):
        ...
        for key, value in params.items():
            ...
            name = value['name']
            indicators[key] = self.init_single_indicator(name, self.previous_data, kwargs)
        return indicators
    ...
    def second_updater(self, indicators_dict, alerts_queue, params, table, ticker_data, alert_updater, current_minute):
        ...
        for key, indicator in indicators_dict.items():
            ticker_indicator_data = indicator.seconds_updater(ticker_data, self.previous_data, ticker_name,
                                                              current_minute)
    ...
    def monitoring_process(self):
        """
            ...
            minute (:05, :10, :15 minutes and etc.)l if we have a '1min' time frame we do this every minute)
        :return:
        """
        while True:
            if not self.minute_queue.empty():
                ...
                for time_frame in updated_timeframes:
                    for indicator in self.time_frames_dict[time_frame]:
                        self.indicators_dict[indicator].prevalues_formula(self.previous_data)
            if not self.aggr_sec_queue.empty():
                ticker_data = self.aggr_sec_queue.get()
                print(ticker_data)
                # if 'AAPL' in ticker_data.keys():
                #     print(ticker_data)
