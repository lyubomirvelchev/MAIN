...
def get_stock_assets(event, context=None):
    try:
        limit = 450
        ...
        tickers_schema_name = 'stock_etf_fundamentals'
        database_connection = mysql.connector.connect(
            host="database-1.clns6yopmify.us-east-1.rds.amazonaws.com",
            user="admin",
            password="dsj89jdj!li3dj2ljefds",
            database=tickers_schema_name
        )
        query = """SELECT general.asset_id, general.Code, general.Name, general.Description, logos.LogoURL
                   FROM {}.general
                   INNER JOIN {}.logos ON general.asset_id=logos.TickerID
                   WHERE general.Type != 'ETF'
                   LIMIT {} OFFSET {};""".format(tickers_schema_name, tickers_schema_name, limit, limit * page_number)
        ...
        for idx in range(len(results)):
            ...
            json_obj[idx]['asset_symbol'] = results[idx][1]
            json_obj[idx]['asset_name'] = results[idx][2]
            json_obj[idx]['asset_description'] = results[idx][3]
            json_obj[idx]['asset_logo'] = results[idx][4]
        return {
